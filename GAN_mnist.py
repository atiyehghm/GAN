...
for epoch in range(num_epochs):
    for n_batch, (real_batch,_) in enumerate(data_loader):
        ...
        real_data = Variable(images_to_vectors(real_batch)).to(device)       # Generate fake data and detach
        # (so gradients are not calculated for generator)
        fake_data = generator(noise(N).to(device)).detach()        # Train D
        d_error, d_pred_real, d_pred_fake = \
              train_discriminator(d_optimizer, real_data, fake_data)

        # 2. Train Generator        # Generate fake data
        fake_data = generator(noise(N).to(device))        # Train G
        ...
        if (n_batch) % 100 == 0:
            ...
            test_images = torch.Tensor.cpu(test_images)
